...
* St_stop is the idle state, whenever the system resets it start from St_stop, and only the red light is high in this state.
...
* After one clock cycle, a transition takes place from state st_redyel to st_green, in state st_green green light is high for 4 additional clock cycles, which means a total of five clock cycles.
